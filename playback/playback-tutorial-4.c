...
static gboolean refresh_ui (CustomData *data) {
  ...
  if (result) {
    ...
    n_ranges = gst_query_get_n_buffering_ranges (query);//need download buff number
    //g_print ("n_ranges: %d\n", n_ranges);
    //for (range = 0; range < n_ranges; range++) {
    if(n_ranges > 0){
      gint64 start, stop;
      gst_query_parse_nth_buffering_range (query, 0, &start, &stop);  //index:0 ,current buffering part.
      //g_print ("start: %ld , stop: %ld \n", start, stop);
      //take buffering_range[index] divide into GRAPH_LENGTH parts.
      /*
       *if index is buffering , its start should 0,and stop is buffered.
       *start  play    stop         end
       * |       |       |           |
       * -----------------------------
       * 0->   buffed  ->|->buffing->|          
