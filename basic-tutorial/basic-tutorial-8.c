...
static int push_buff_num = 0;
static int push_buff = 0;
static int pull_buff_num = 0;
...
static gboolean push_data (CustomData *data) {
 ...
 /* gsize buff_size = gst_buffer_get_sizes (buffer,NULL,NULL);
  g_print ("push-buffer: %ld ", buff_size);*/
  g_print (" %d ", ++ push_buff_num );
...
static void start_feed (GstElement *source, guint size, CustomData *data) {
  ...
  if(2 == push_buff ++){
      g_signal_emit_by_name (data->app_source, "end-of-stream", NULL);
  }
}
...
static GstFlowReturn new_sample (GstElement *sink, CustomData *data) {
  ...
  if (sample) {
    ...
    gsize buff_size = gst_buffer_get_sizes (buff,NULL,NULL);
    g_print (" %ld ", buff_size);
    gst_buffer_ref(buff);*/
    //g_print (" %d  ", ++ pull_buff_num );
...
int main(int argc, char *argv[]) {
  ...
  /* Configure appsink */
  //appsink emit-signals Default: false
